...
function Auth() {
  ...
  const handleOtpChange = (index, value) => {
    ...
    // If last index filled and all digits present, auto verify
    if (value !== "" && index === 3) {
      const candidateOtp = newOtp.join("");
      const hasAllDigits =
        candidateOtp.length === 4 && !newOtp.some((d) => d === "");
      if (hasAllDigits && !verifyLoading) {
        // Trigger verification without requiring Enter or button click
        handleVerifyOTP();
      }
    }
  };
